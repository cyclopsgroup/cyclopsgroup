    ...
    implements Action
{
    ...
    public void execute( RuntimeData data, ActionContext context )
        throws Exception
    {
        String[] teacherIds = data.getParameters().getStrings( "teacher_id" );
        PersistenceManager persist = (PersistenceManager) lookup( PersistenceManager.ROLE );
        for ( int i = 0; i < teacherIds.length; i++ )
        {
            String teacherId = teacherIds[i];
            persist.delete( Teacher.class, teacherId );
        }
        context.addMessage( teacherIds.length + " teachers are deleted" );
    }
}
